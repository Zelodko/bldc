...
int16_t canard_process_frame(CANRxFrame *msg, int interface) {
	...
	if (msg->common.IDE == CAN_IDE_EXT) {
		rx_frame.id = msg->ext.EID | CANARD_CAN_FRAME_EFF;
	} else {
		rx_frame.id = msg->std.SID;
	}
	...
	if (interface == 1) {
		return canardHandleRxFrame(&canard_ins, &rx_frame, TIME_I2US(chVTGetSystemTimeX()));
	} else {
#ifdef HW_CAN2_DEV
		return canardHandleRxFrame(&canard_ins_if2, &rx_frame, TIME_I2US(chVTGetSystemTimeX()));
#endif
	}
