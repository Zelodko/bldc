...
static bool lib_set_cfg_float(CFG_PARAM p, float value) {
	...
	*appconf = *app_get_configuration();
	...
	switch (p) {
		...
		// Upstream release_7_00 has a copy-paste bug here (assigns to foc_motor_r
		// instead of foc_motor_l) - using the obviously-intended field instead.
		case CFG_PARAM_foc_motor_l: mcconf->foc_motor_l = value; changed_mc = 1; res = true; break;
...
// vesc_c_if's can_transmit_sid/eid are void-returning for ABI stability
// with already-compiled packages, but comm_can_transmit_sid/eid now
// return msg_t - these wrappers discard the result to bridge the two.
