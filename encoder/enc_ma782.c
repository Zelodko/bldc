...
static bool ma782_read_angle(ma782_config_t *cfg) {
	...
	//
	// H7's SPI peripheral splits the F4-era single DR register into separate
	// TXDR/RXDR registers - RXDR is the receive-side equivalent here.
...
// TODO: not hardware-tested on H7 - this driver was ported from upstream release_7_00
// verbatim (only the SPI DR->RXDR register rename and a PUDR->PUPDR macro name fix were
// needed to compile). No MA782 hardware exists for this port yet; verify SPI timing/CPOL-
// CPHA and the compute_ma782_callback wiring in encoder_cfg.c against real hardware.
bool enc_ma782_init(ma782_config_t *cfg) {
	...
	palSetPadMode(cfg->miso_gpio, cfg->miso_pin,
			PAL_MODE_ALTERNATE(cfg->spi_af) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUPDR_FLOATING);
