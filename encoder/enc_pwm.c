...
// TODO: not hardware-tested on H7 - ported from upstream release_7_00 verbatim (only the
// missing ICUConfig.arr field was added; upstream's initializer omitted it, which left ARR
// defaulting to 0 instead of the documented 0xFFFFFFFFU, breaking capture range - same class
// of bug fixed earlier in lispif_vesc_extensions.c's ICUConfig usage). No PWM/PWM_ABI
// encoder hardware exists for this port yet; verify ICU capture timing on real hardware.
