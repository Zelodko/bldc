...
// m_transport.bus.spi_hw.txd/rxd are DMA source/destination buffers for transport_spi_hw.c.
// Plain .bss/.data on this port lands in DTCM (see ld_eeprom_emu_h7.ld: BSS_RAM/DATA_RAM alias
// to ram5), which is core-local and physically unreachable by any DMA controller on H7 - every
// DMA transfer targeting it fails with a transfer error (confirmed on hardware: this is what was
// tripping STM32_SPI_DMA_ERROR_HOOK's osalSysHalt("DMA failure"), which this port wires to reset
// the MCU, producing a boot-crash-reset loop that looked like a total freeze). .nocache (ram3/
// AXI SRAM3) is this codebase's established DMA-buffer placement, already used the same way for
// ADC_Value in mc_interface.c.
...
static void imu_read_callback(float *accel, float *gyro, float *mag) {
	...
	// ST2MS doesn't exist in this ChibiOS version - TIME_I2MS is the equivalent for a
	// systime_t tick delta (see the same fix already applied elsewhere in this port).
